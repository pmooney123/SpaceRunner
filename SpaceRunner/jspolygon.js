     ...
     */
    Polygon.prototype.draw = function(ctx, cameraX, cameraY) {

        ctx.save();
        ctx.fillStyle = this.color;
        ctx.beginPath();
        ctx.moveTo(this.points[0].x + this.center.x - cameraX, this.points[0].y + this.center.y - cameraY);
        for (var i = 1; i < this.points.length; i++) {
        	ctx.lineTo(this.points[i].x + this.center.x - cameraX, this.points[i].y + this.center.y - cameraY);
        }
        ctx.closePath();
        ctx.fill();
        ctx.restore();
